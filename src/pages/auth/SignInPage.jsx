...
function SignInPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (isChecked) {
        storeUserToken({ accessToken: token });
        if (user) {
          storeUserInfo(user);
        }
      }
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              <div className="w-full">
                ...
                <input
                  ...
                  className="w-full px-5 py-3 border-2 border-[#111826] rounded-md outline-none mt-5 placeholder:text-xl"
                  required
                />
              </div>
              <div className="w-full">
                ...
                <div className="w-full relative">
                  <input
                    ...
                    className="w-full border-2 border-[#111826] rounded-md outline-none px-5 py-3 mt-5 placeholder:text-xl"
                    required
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-3 bottom-4 flex items-center text-[#111826]"
                  >
                    {showPassword ? (
              ...
              <div className="flex justify-center items-center">
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-1/3 bg-[#111826] text-white font-bold py-3 rounded-lg shadow-lg cursor-pointer mt-5 disabled:opacity-60 disabled:cursor-not-allowed"
                >
